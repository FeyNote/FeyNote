  ...
  const graphArtifacts = useMemo(() => {
    return artifacts
      .filter((artifact) => {
        ...
        return artifact.artifactReferences.length || artifact.incomingArtifactReferences.length
      })
      .map((artifact) => ({
        id: artifact.id,
  ...
  return (
    <IonPage>
      ...
      {isPaneFocused &&
        ...
        createPortal(
          <GraphRightSidemenu
            lockedArtifacts={graphArtifacts.filter((artifact) => artifactPositions.has(artifact.id))}
            unlockArtifact={(id) => {
              artifactsYKV.set(id, {
                lock: null,
              });

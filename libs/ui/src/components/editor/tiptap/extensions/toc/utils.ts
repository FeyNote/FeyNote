...
export const getHeadlineLevel: GetTableOfContentLevelFunction = (
  headline,
  previousItems,
) => {
  let level = 1;
  ...
  if (headline.node.attrs.level > (previousHeadline?.originalLevel || 1)) {
    ...
    headline.node.attrs.level < (previousHeadline?.originalLevel || 1)
  ) {
...
export const getLinearIndexes: GetTableOfContentIndexFunction = (
  _headline,
  previousHeadlines,
) => {
...
export const getHierarchicalIndexes: GetTableOfContentIndexFunction = (
  ...
  const previousHeadlinesOnLevelOrBelow = previousHeadlines.filter(
    (h) => h.level <= level,
  );

...
function deduplicate(array: any[]) {
...
export interface UniqueIDOptions {
  ...
  types: string[];
  generateID: () => string;
  filterTransaction?: ((tr: any) => boolean) | null;
}
...
const UniqueID = Extension.create<UniqueIDOptions>({
          ...
          [this.options.attributeName]: {
            ...
            renderHTML: (attrs) =>
              attrs[this.options.attributeName]
                ? { [`data-${this.options.attributeName}`]: attrs[this.options.attributeName] }
                : {},
          },
        },
      },
  ...
  onCreate() {
    ...
    findChildren(doc, (node) => types.includes(node.type.name) && node.attrs[attributeName] == null)
      .forEach(({ node, pos }) => {
    ...
    return [
      new Plugin({
        ...
        appendTransaction(transactions, oldState, newState) {
          ...
          const isSyncTransaction = transactions.find((tr) => tr.getMeta("y-sync$"));

          if (isSyncTransaction || !docChanged || shouldSkip) return;

          const tr = newState.tr;
          const { types, attributeName, generateID } = this.options;
          const { mapping } = combineTransactionSteps(oldState.doc, transactions as Transaction[]);
          const changedRanges = getChangedRanges({
            mapping
          } as Transform);

          changedRanges.forEach(({ newRange }) => {
            const nodesInRange = findChildrenInRange(newState.doc, newRange, (node) =>
          ...
          return tr.steps.length ? (tr.setStoredMarks(newState.tr.storedMarks), tr) : undefined;
        },

        view(editorView) {
          const handleDragStart = (event: DragEvent) => {
            const parent = editorView.dom.parentElement;
            dragContext = parent?.contains(event.target as Node | null) ? parent : null;
          };

...
export const MultiArtifactDeleteDialog: React.FC<Props> = (props) => {
  ...
  const deleteAction = async () => {
    ...
    for (const artifactId of props.artifactIds) {
      await deleteArtifactOrRemoveSelf(artifactId).then(() => {
        successCount++;
      }).catch((e) => {
        failedCount++;
  ...
  const processingStatusDialog = (
    <ActionDialog
      ...
      description={`${
        t('allArtifacts.deleteMultiple.deleting.message', {
          totalCount: resultStats?.workingSetSize,
          successCount: resultStats?.success,
        })
      } ${
        !!resultStats?.failed ? t('allArtifacts.deleteMultiple.deleting.message.failed', {

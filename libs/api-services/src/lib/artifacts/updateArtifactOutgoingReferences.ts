...
export async function updateArtifactOutgoingReferences(
  ...
  await tx.artifactReferenceDisplayText.createMany({
    data: addedBlocksArtifactReferences
      .map((newBlockReference) => ({
  ...
  await tx.artifactBlockReferenceDisplayText.createMany({
    data: addedBlocksArtifactBlockReferences
      .map((newBlockReference) => ({
        artifactId: newBlockReference.targetArtifactId,
  ...
  const referencedArtifactIds = [
    ...artifactReferences.map((artifactReference) => artifactReference.targetArtifactId),
    ...artifactBlockReferences.map((artifactReference) => artifactReference.targetArtifactId),
  ];
  ...
  const connectableReferencedArtifactIds = new Set(connectableReferencedArtifacts.map((connectableReferencedArtifact) => connectableReferencedArtifact.id));
  ...
  await tx.artifactReference.createMany({
    data: artifactReferences.map((reference) => ({
      ...
      referenceTargetArtifactId: connectableReferencedArtifactIds.has(reference.targetArtifactId) ? reference.targetArtifactId : undefined,

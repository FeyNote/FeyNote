...
export function updateReferencedAnchors(
  ...
  for (const aEl of aEls) {
    ...
    blockEl.setAttribute('data-artifact-block-id', blockInfo?.id || BROKEN_REFERENCE_ID);
    blockEl.setAttribute('data-artifact-id', blockInfo?.artifactId || BROKEN_REFERENCE_ID);

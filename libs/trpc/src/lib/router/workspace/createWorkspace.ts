...
import { syncWorkspaceRelations } from '@feynote/api-services';
    ...
    async ({
      ...
      const workspace = await prisma.$transaction(
        async (tx) => {
          ...
          const artifactsKV = getWorkspaceArtifactsFromYDoc(yDoc);
          const threadsKV = getWorkspaceThreadsFromYDoc(yDoc);
          const userAccessKV = getWorkspaceUserAccessFromYDoc(yDoc);
          ...
          await syncWorkspaceRelations({
            ...
            currentUserAccess: [...userAccessKV.yarray.toArray()],
          });
